 ...
 */
export const pendingBranchSnapshots = new Map();
/**
 ...
 */
function resolveBashWorkdir(args, projectCwd) {
    const workdirArg = args?.workdir;
    if (workdirArg) {
 ...
 */
export function isBranchChangeCommand(args) {
 ...
 */
function extractCheckoutPaths(args, workdir) {
    const command = args?.command || '';
    if (!command.trim()) {
        return [];
    }
 ...
 */
function getCurrentHeadRef(workdir) {
    try {
 ...
 */
function getChangedPathsBetweenRefs(workdir, prevRef, nextRef) {
    if (prevRef === nextRef) {
        return [];
    }
    try {
 ...
 */
function extractMutatedPaths(tool, output, args) {
    const paths = [];
 ...
 */
function isPathInProject(absPath, cwd) {
 ...
 */
function isWorkdirInProject(workdir, projectCwd) {
 ...
 */
export function createGraphToolBeforeHook(deps) {
...
export function createGraphToolAfterHook(deps) {
    return async (input, output) => {
        // No-op if graph service is disabled
        if (!deps.graphService) {
            return;
