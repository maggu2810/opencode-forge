...
export function createSandboxToolBeforeHook(deps) {
    return async (input, 
    ...
    output) => {
        ...
        if (input.tool === 'bash') {
            ...
            try {
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`sandbox hook timeout after ${hookTimeout}ms`)), hookTimeout));
        ...
        if (input.tool === 'grep') {
            ...
            try {
                const result = await executeSandboxGrep({ docker, containerName, hostDir }, args.pattern, { path: args.path, include: args.include });
                pendingResults.set(input.callID, { result, storedAt: Date.now() });
            }
            catch (err) {
                const message = err instanceof Error ? err.message : String(err);
...
export function createSandboxToolAfterHook(deps) {
    return async (
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- matches upstream Hooks type
    input, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- matches upstream Hooks type
    output) => {
        if (input.tool !== 'bash' && input.tool !== 'glob' && input.tool !== 'grep')
            return;
        const now = Date.now();
